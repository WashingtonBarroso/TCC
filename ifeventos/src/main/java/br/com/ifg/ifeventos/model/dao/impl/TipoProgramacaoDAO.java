...
public class TipoProgramacaoDAO extends DAO<TipoProgramacao, Long> {
	...
	public List<TipoProgramacao> search(BootstrapTableParamsDTO params){
		...
		return (List<TipoProgramacao>) this.getPageableList(params.getOffset(),"from TipoProgramacao tp where tp.descricao like :param or tp.id = :id order by tp."+params.getSort()+" "+params.getOrder(),
				"id",id,
	...
	public Long count(BootstrapTableParamsDTO params){
		...
		return (Long) this.getGenericList("select count(*) from TipoProgramacao where descricao like :param or id = :id",
				"id",id,
